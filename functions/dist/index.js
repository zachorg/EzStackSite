...
async function getPepper() {
    ...
    if (!pepperResource)
        throw new Error('APIKEY_PEPPER_RESOURCE not set');
    cachedPepper = await accessSecret(pepperResource);
    return cachedPepper;
}
// Optional: envelope encryption for demo key material
async function encryptWithKms(kmsKeyResource, plaintext) {
...
async function decryptWithKms(kmsKeyResource, ciphertext) {
...
function generateBase32NoAmbiguous(length) {
...
export const demoProxyCall = https.onRequest({ cors: true, region: process.env.FUNCTIONS_REGION || 'us-central1' }, async (req, res) => {
    try {
        ...
        const kmsKey = process.env.KMS_KEY_RESOURCE;
        if (!kmsKey)
            throw new HttpsError('failed-precondition', 'KMS not configured');
        ...
        if (data.userId !== uid)
            throw new HttpsError('permission-denied', 'Forbidden');
        if (!data.keyMaterialEnc)
            throw new HttpsError('failed-precondition', 'Not a demo key');
